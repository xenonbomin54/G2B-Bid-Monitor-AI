...
- v12 경쟁제품이 아닌데 **직접생산확인을 요구**하면 위반.
  ※ 경쟁제품 입찰이면 직접생산확인 요구는 **적법**이므로 v12 가 아니다(오히려 v10 쪽이다).
...
**법정 예외 — v15·v16·v17·v18 에 모두 적용된다.**
아래에 해당하는 사유가 공고문에 적혀 있으면, 기업규모 제한을 하지 않았거나 위 구간과
다르게 했더라도 **위반이 아니다**(판로지원법 시행령 제2조의3제2항이 사유를 공고문에
기재하도록 정하고 있으므로, 공고문에서 사유를 찾으면 된다).
- 학술·연구·조사·검사·평가·개발 등 지적 활동을 통한 자문 용역으로 비영리법인 참가가 필요한 경우
- 학교 교육과정 외 교육활동 위탁, 검체검사 위탁, 보조사업 수행 위탁 용역
- 다른 법령이 우선구매대상으로 정했거나 수의계약·지명경쟁을 허용한 경우
  (중증장애인생산품·사회적기업·여성기업 우선구매 등)
- 특정한 성능·기술·품질이 필요해 기업규모를 제한해서는 목적 달성이 불가능한 경우
- 소기업·소상공인이 3인 이하이거나 유찰된 경우
  → 이때는 1억원 미만이어도 **중소기업자 제한이 적법**하다(v17 성립 불가).
사유가 적혀 있지 않다면 예외로 보지 않는다.""",
    ),
    Group(
        key="G3물품SW공동",
        ...
        full_doc=True,
        지시="""판정 기준
- v9 특정 모델명: 규격서·과업지시서가 **조달하려는 물품 자체**를 특정 제조사·브랜드·
  모델명으로 못 박으면 위반. "동등 이상", "동급 제품 가능" 같은 대체 허용 문구가
  함께 있으면 위반이 아니다.
  위반이 **아닌** 것 —
  · 용역 수행에 쓰는 도구·검사장비·프로그램 이름 (예: 검사도구 MMPI-2, 측정기 모델명)
  · 기존 시스템과의 호환 요건으로 언급된 제품명 (예: 기존 OS와 동일 제조사)
  · 유지보수 대상이 되는 이미 도입된 제품의 이름
...
- v24 공고서와 나라장터 등록값 상이: [나라장터 등록정보]의 값과 공고문 본문의 기재가
  **실제로 다른 값**일 때만 위반. 대조 대상은 예산·추정가격, 계약방법, 낙찰방법, 지역제한, 업종제한.
  ...
  위반이 **아닌** 것 — 아래는 v24 가 아니다. 다른 항목에서 다룬다.
  · 지역제한·소재지 요건이 적혀 있다는 사실 (등록값과 다르지 않다면 v24 아님 → v5~v7)
  · 기업규모(중소기업·소기업) 자격 요건이 적혀 있다는 사실 (→ v13~v18)
  · 실적·면허 요건이 적혀 있다는 사실 (→ v2~v4)
  이 항목은 **두 값을 견주어 서로 다를 때만** 1이다. 요건의 적법성은 여기서 따지지 않는다.""",
    ),
]
...
판정 원칙 — 입찰공고는 담당자가 법령을 참고해 작성한 문서이므로 대부분 적법하다.
위반은 예외적으로 발견되는 것이다.
- **모든 항목의 기본값은 0이다.** 1은 문서가 위반을 직접 보여줄 때만 낸다.
- "위반일 수도 있다", "그렇게 읽힐 여지가 있다" 정도로는 **0**이다.
  근거가 될 문장을 지목할 수 없으면 0이다.
- **관련 문구가 있다는 사실만으로는 위반이 아니다.** 실적을 요구했다고 곧 위반이 아니고,
  그 요구가 이 금액대에서 법이 허용하는 범위를 **넘었는지**를 봐야 한다.
- 항목을 하나씩 볼 때 "이것도 걸릴 수 있나?"를 반복하면 실제보다 훨씬 많이 찍게 된다.
  판정을 마친 뒤 1이 여러 개라면, 각각이 정말 근거를 갖췄는지 다시 확인하라.
